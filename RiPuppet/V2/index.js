...
(async () => {
  ...
  for (b of browsers) {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------------
    // Login first
    let elementListLogin = [];
    await page.goto(baseUrl, { waitUntil: "networkidle" }).catch((err) => {
      console.log(err);
      return;
    });
    await getTextInputs(page, elementListLogin);
    await getButtons(page, elementListLogin);
    // Element deleted because the first button is Forgot option
    elementListLogin.splice(2, 1);
    await interactWithObjects(elementListLogin, page, 0, "");

    // Back delay to avoid errors
...
async function scrapLinks(page) {
  const stories = await page.evaluate(() => {
    ...
    const linksFiltered = links.filter(
      (link) =>
        !link.toString().includes("#/site") &&
        !link.toString().includes("#/settings") &&
        !link.toString().includes("#/dashboard")
    );
    const webLinks = linksFiltered.filter((link) =>
      link.toString().includes("http")
    );
...
async function getTextInputs(page, elementList) {
  ...
  for (let i = 0; i < textInputs.length; i++) {
    let disabled = await page.evaluate((el) => {
      if (el.getAttribute("data-test-input")?.includes("-search")) return true;
      return false;
    }, textInputs[i]);
    if (!disabled) {
      input = {
  ...
  for (let i = 0; i < buttons.length; i++) {
    let disabled = await page.evaluate((btn) => {
      return (
        ...
        btn.getAttribute("class") === "gh-nav-btn-search" || //barra de búsqueda
        btn.getAttribute("class") === "gh-nav-button-expand " || //botones expandibles (ej. post)
        btn.getAttribute("class") === "gh-nav-button-expand" ||
        btn.getAttribute("class") === "mr2" ||
        btn.getAttribute("class") === "gh-btn-expand" ||
        btn.getAttribute("data-test-button") === "members-actions" // importar y exportar miembros
      );
    }, buttons[i]);
    if (!disabled) {
      button = {
...
async function fillInput(elementHandle, page) {
  let type = await page.evaluate((el) => {
    if (el.getAttribute("data-test-input")?.includes("-search")) return null;
    return el.type;
  }, elementHandle);
  if (type === "text") {
    elementHandle.click();
